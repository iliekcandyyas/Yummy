...
async def get_ai_reply(messages: list, ai: str) -> str:
    global groq_client
    system = "You are a helpful assistant in a Discord server. Be concise and friendly. you may be a bit rude to message.author.id == 1304112685599690863 and message.author.id == 1348737415765954726"
...
bot.run(token)groq_client = Groq(api_key=GROQ_API_KEY)
import time
bot.launch_time = time.time()
